...
export const PlaybackScrubber: React.FC<PlaybackScrubberProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-auto bg-white/95 backdrop-blur-md border border-[#e3e7ec] shadow-xl rounded-full px-4 h-full flex items-center space-x-3 text-xs text-slate-800 max-w-xl w-full">
        {/* Live Pill Toggle */}
        <button
          onClick={handleGoLive}
          className={`flex items-center space-x-1.5 px-3 py-1 rounded-full text-xs font-bold transition-editorial cursor-pointer uppercase ${
            isLive
        ...
        {/* Play/Pause toggle when scrubbing */}
        {!isLive && (
          <button
            onClick={() => setIsPlaying(!isPlaying)}
            className="w-7 h-7 rounded-full bg-slate-900 text-white flex items-center justify-center hover:bg-slate-800 transition-editorial cursor-pointer shadow-xs shrink-0"
        ...
        {/* Scrub Offset Tag */}
        <div className="text-[11px] font-mono font-semibold text-slate-600 tabular-nums shrink-0 min-w-[50px]">
        ...
        <div className="flex-1 relative flex items-center h-6">
          {/* Timeline Bar Track */}
          <div className="w-full h-1.5 bg-slate-200 rounded-full relative overflow-visible">
